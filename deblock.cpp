...
int judge_filter(int p1,int p0,int q0,int q1,int index_a,int index_b,int thres) {
    if ((p1+p0+q0+q1)/4 > thres) return 0;  // The block artifacts of high brightness area is not obvious
    if (abs(p0-q0) < alpha(index_a) && abs(p1-p0) < beta(index_b) && abs(q1-q0) < beta(index_b)) return 1;
    else return 0;
}
...
bool judge_same(LINE line1, LINE line2) {
    if (line1.z == line2.z && line1.l == line2.l && line1.r == line2.r 
        && line1.d == line2.d && line1.u == line2.u) return true;
    else return false;
}
...
void deblock(std::string step_dir, uint32_t index_a, uint32_t index_b, uint32_t thres) {
    ...
    for (uint16_t i=0; i < block_infos.size(); i++) {
        ...
        for (uint16_t i=0; i < lines.size(); i++) {
            if (judge_same(lines[i], line_value(lline, z1, x1, x1, y1, y2))) lflag = true;
            if (judge_same(lines[i], line_value(rline, z1, x2, x2, y1, y2))) rflag = true;
            if (judge_same(lines[i], line_value(dline, z1, x1, x2, y1, y1))) dflag = true;
            if (judge_same(lines[i], line_value(uline, z1, x1, x2, y2, y2))) uflag = true;
        }
        for (uint16_t z=z1; z < z2+1; z++) {
            if (!lflag) lines.push_back(line_value(line, z, x1, x1, y1, y2));
            if (!rflag) lines.push_back(line_value(line, z, x2, x2, y1, y2));
            if (!dflag) lines.push_back(line_value(line, z, x1, x2, y1, y1));
            if (!uflag) lines.push_back(line_value(line, z, x1, x2, y2, y2));
        }
    }
    // deblock
    ...
    for (uint16_t i=0; i < lines.size(); i++) {
        ...
        if ((line.l == line.r) && (line.l - 3 < 0 || line.l + 3 > W -1)) continue;
        else if ((line.d == line.u) && (line.d - 3 < 0 || line.d + 3 > H -1)) continue;
            ...
            for (uint16_t x = line.l; x < line.r + 1; x++) {
                if (line.l == line.r) {
                    ...
                    if (judge_filter(p1,p0,q0,q1,index_a,index_b,thres)) {
                        ...
                        img[line.z*H*W+y*W+x-2] = results[0]; img[line.z*H*W+y*W+x-1] = results[1];
                        img[line.z*H*W+y*W+x+0] = results[2]; img[line.z*H*W+y*W+x+1] = results[3];
                    }
                    ...
                    if (judge_filter(p1,p0,q0,q1,index_a,index_b,thres)) {
                        results = filter(p2,p1,p0,q0,q1,q2,index_b);
                        img[line.z*H*W+(y-2)*W+x] = results[0]; img[line.z*H*W+(y-1)*W+x] = results[1];
                        img[line.z*H*W+(y+0)*W+x] = results[2]; img[line.z*H*W+(y+1)*W+x] = results[3];
                    }
                }
            }
        }
