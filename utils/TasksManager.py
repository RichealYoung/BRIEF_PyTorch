...
class Worker():
    def __init__(self,gpu_list:List[int],task: Task):
        ...
        self.command_refine = task.command+' -g {}'.format(','.join([str(i) for i in gpu_list]))
    def start(self,debug=False):
        if debug:
            self.process = Popen(self.command_refine,shell=True)
        else:
            self.process = Popen(self.command_refine,shell=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,)
    ...
    def get_gpu_memory_bbncr(self,pid:int):
    ...
    def query_gpuusing_cpuusing(self,):
        ...
        self.query_children()
        for proc in self.children:
            self.gpuusing += self.get_gpu_memory_bbncr(proc.pid) # MBytes
            if psutil.pid_exists(proc.pid):
                self.cpuusing += psutil.Process(proc.pid).memory_info().rss/2**20 # MBytes
...
class Queue():
    ...
    def run_pending_list(self,max_task=1e8,debug=False,autogpu=True):
        ...
        for task in self.pending_list:
            ...
            if autogpu:
                ...
                task.worker.start(debug)
                logging.warning('Task:{} Start! Using GPU:{}'.format(task.name,','.join([str(i) for i in self.gpu_list])))
                temp_task_list.append(task)
                time.sleep(1) # keep 5 sec to avoid crowded line
    ...
    def log(self,):
        ...
        running_table.field_names=['Name','CraeteTime','gpu idx','gpu using/cost(MB)','cpu using/cost(MB)','pids',]
        for task in self.running_list:
            running_table.add_row([
            task.name,
            datetime.datetime.fromtimestamp(task.worker.create_time).strftime("%Y-%m-%d %H:%M:%S"),
            ','.join([str(i) for i in self.gpu_list]),
            '{:.0f}/{:.0f}'.format(task.gpuusing,task.gpucost),
            '{:.0f}/{:.0f}'.format(task.cpuusing,task.cpucost),task.worker.pids
        ])
        print(running_table)
        cost_table = PrettyTable()
        cost_table.title = 'cost variable'
        cost_table.field_names=['cost variable','gpu cost(MB)','cpu cost(MB)',]
        for cv in self.sharecost_dict:
            cost_table.add_row([
        ...
        for idx in self.gpufree:
            sysinfo.add_column('gpu{}_free(MB)'.format(idx),['{:.0f}'.format(self.gpufree[idx])])
        print(sysinfo)
